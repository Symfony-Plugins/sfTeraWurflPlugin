...
class TeraWurflLoader{
	...
	// Constructor
	public function __construct(sfTeraWurfl &$wurfl,$source){
		$this->errors = array();
		$this->wurfl = $wurfl;
		$this->source = $source;
		$this->devices = array();
	...
	public function validate(){
		...
		foreach($this->xml->devices->device as $device){
			...
			if($this->table == sfTeraWurflConfig::getDeviceTableNamePrefix()){
	...
	public function loadPatches(){
		if(!sfTeraWurflConfig::isCacheEnabled()) return true;
		...
		$patches = explode(';',sfTeraWurflConfig::getPatchFile());
		foreach($patches as $patch){
			$this->wurfl->toLog("Loading patch: ".$patch,LOG_WARNING);
			$this->loadPatchFile(sfTeraWurfl::getAbsoluteDataDir().$patch);
		}
		return true;
	}
		...
		foreach($this->patchtables as $table => &$devices){
			...
			foreach($devices as $id => &$device){
				if(isset($maintable[$id])){
					...
					sfTeraWurfl::mergeCapabilities($maintable[$device['id']],$device);
	...
	private function getFile(){
			...
			case self::$WURFL_PATCH:
				return sfTeraWurfl::getAbsoluteDataDir().sfTeraWurflConfig::getPatchFile();
				break;
			case self::$WURFL_LOCAL:
				return sfTeraWurfl::getAbsoluteDataDir().sfTeraWurflConfig::getDataFile();
				break;
		}
	}
	private function getTable(){
		switch($this->source){
			case self::$WURFL_PATCH:
				return sfTeraWurflConfig::getPatchFile();
				break;
			case self::$WURFL_LOCAL:
			case self::$WURFL_REMOTE:
			case self::$WURFL_REMOTE_CVS:
			default:
				return sfTeraWurflConfig::getDeviceTableNamePrefix();
				break;
		}
	}
