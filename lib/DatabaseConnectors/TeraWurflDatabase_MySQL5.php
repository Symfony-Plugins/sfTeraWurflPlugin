 ...
 * $Id: TeraWurflDatabase_MySQL5.php,v 1.12 2008/03/01 00:05:25 kamermans Exp $
 * $RCSfile: TeraWurflDatabase_MySQL5.php,v $
 * 
...
class TeraWurflDatabase_MySQL5 extends TeraWurflDatabase{
	...
	public function getDeviceFromID($wurflID){
		$this->numQueries++;
		$res = $this->dbcon->query("SELECT * FROM `".sfTeraWurflConfig::getMergeTable()."` WHERE `deviceID`=".$this->SQLPrep($wurflID)) or die($this->dbcon->error);
		if($res->num_rows == 0){
			$res->close();
	...
	public function getDeviceFromUA($userAgent){
		$this->numQueries++;
		$query = "SELECT `deviceID` FROM `".sfTeraWurflConfig::getMergeTable()."` WHERE `user_agent`=".$this->SQLPrep($userAgent);
...
//		die("DATA: ".var_export($data,true));
	...
	public function loadDevices(&$tables){
		...
		$insertcache = array();
		$insertedrows = 0;
		$this->createIndexTable(sfTeraWurflConfig::getIndexTable());
		...
		foreach($tables as $table => $devices){
			...
			foreach($devices as $device){
				$this->dbcon->query("INSERT INTO `".sfTeraWurflConfig::getIndexTable()."` (`deviceID`,`matcher`) VALUE (".$this->SQLPrep($device['id']).",".$this->SQLPrep($matcher).")");
	 ...
	 *
	 * @param string Table name (ex: sfTeraWurflConfig::$HYBRID)
	...
	private function clearMatcherTables(){
		foreach(UserAgentFactory::$matchers as $matcher){
			$table = sfTeraWurflConfig::getDeviceTableNamePrefix()."_".$matcher;
	...
	public function createMergeTable($tables){
		$tablename = sfTeraWurflConfig::getMergeTable();
	 ...
	 * Drops and creates the given device table
	 *
	 * @param string Table name (ex: sfTeraWurflConfig::$INDEX)
	 * @return boolean success
	 */
	public function createIndexTable(){
		$tablename = sfTeraWurflConfig::getIndexTable();
		$droptable = "DROP TABLE IF EXISTS ".$tablename;
		$createtable = "CREATE TABLE `".$tablename."` (
  `deviceID` varchar(128) binary NOT NULL default '',
  ...
  PRIMARY KEY  (`deviceID`)
	...
	public function getDeviceFromCache($userAgent){
		$tablename = sfTeraWurflConfig::getCacheTable();
		$this->numQueries++;
	...
	public function createTempCacheTable(){
		$tablename = sfTeraWurflConfig::getCacheTable().self::$DB_TEMP_EXT;
		$droptable = "DROP TABLE IF EXISTS `$tablename`";
		$createtable = "CREATE TABLE `$tablename` (
			`user_agent` varchar(255) binary NOT NULL default '',
			`cache_data` mediumtext NOT NULL,
	...
	public function rebuildCacheTable(){
		...
		$rebuilder = new sfTeraWurfl();
		$cachetable = sfTeraWurflConfig::getCacheTable();
		$temptable = sfTeraWurflConfig::getCacheTable().self::$DB_TEMP_EXT;
...
//			$rebuilder->toLog($query,LOG_ERR,"rebuildCacheTable");
	...
	public function clearTable($tablename){
		if($tablename == sfTeraWurflConfig::getCacheTable()){
			$this->createCacheTable();
		}else{
			$this->createGenericDeviceTable($tablename);
		}
...
END";
	...
	public function connect(){
		...
		$this->dbcon = new mysqli(
				sfTeraWurflConfig::getDataBaseHost(),
				sfTeraWurflConfig::getDataBaseUserName(),
				sfTeraWurflConfig::getDataBasePassword(),
				sfTeraWurflConfig::getDataBaseName() 
			);
	...
	public function getCachedUserAgents(){
		$uas = array();
		$cacheres = $this->dbcon->query("SELECT user_agent FROM ".sfTeraWurflConfig::getCacheTable()." ORDER BY user_agent");
